...
try:
    ...
    # NEW: Reverse-Lookup Dictionary (Maps Descriptions back to Codes)
    desc_to_code_dict = dict(zip(
        activity_ref['Reference Description'].astype(str).str.strip().str.lower(), 
        activity_ref['Reference Code Column'].astype(str).str.zfill(5)
    ))
    ...
    if export_file:
        with st.spinner('Preparing Buildlogic export...'):
            try:
                ...
                # --- BULLETPROOF EXTRACTION & AUTO-LOOKUP LOGIC ---
                ...
                raw_codes = parsed_activities.apply(lambda x: x[0])
                bl['Description'] = parsed_activities.apply(lambda x: x[1])
                
                # Automatically look up codes for descriptions missing a prefix
                lower_desc = bl['Description'].str.lower().str.strip()
                inferred_codes = lower_desc.map(desc_to_code_dict)
                
                # Merge explicitly provided codes and inferred codes
                final_codes = raw_codes.replace('', np.nan).fillna(inferred_codes)

                bl['ReferenceCode'] = pd.to_numeric(final_codes, errors='coerce')
                ...
                # VLOOKUPs now safely use the final merged codes
                bl['Trade'] = final_codes.astype(str).str.zfill(5).map(act_trade_dict)
                bl['CostCode'] = final_codes.astype(str).str.zfill(5).map(act_cost_dict)
